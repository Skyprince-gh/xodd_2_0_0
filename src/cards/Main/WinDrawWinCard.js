...
import Create from '../Create/WinDrawWin'
import {format} from 'date-fns'

function WinDrawWinCard({ data }) {
    ...
    <div>
      { !eventsWindowIsActive &&
        <div className={oddsWindowStyle} id={newCard.id} onClick={(e) => toggleEditCardWindow(newCard.id)}>
          ...
          <div className={style.grp}>
            <h3>Prediction</h3>
            <p>{data.win_draw_win.prediction}</p>
          </div>

          <div className={style.grp}>
            <h3>Percentage</h3>
            <p>{data.win_draw_win.percentage}%</p>
          </div>
          ...
          <div className={style.grp + " " + style.odds_grp}>
            ...
            <div className={style.odds}>
              <div className={style.odd}>
                <h4>Home</h4>
                <p>{data.win_draw_win.odds.home}</p>
              </div>
              <div className={style.odd}>
                <h4>Draw</h4>
                <p>{data.win_draw_win.odds.draw}</p>
              </div>
              <div className={style.odd}>
                <h4>Away</h4>
                <p>{data.win_draw_win.odds.away}</p>
              </div>
            </div>
          </div>
          ...
          <div className={style.grp_scores}>
            ...
            {data.win_draw_win.isWon && <div className={style.validation_green}></div>}

            {!data.win_draw_win.isWon && <div className={style.validation_red}></div>}
          </div>
        </div>
      }
