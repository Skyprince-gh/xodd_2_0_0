...
import Create from '../Create/OverZeroPointFive'
import {format} from 'date-fns'

function OverZeroPointFive({ data }) {
  // ////console.log('original data:', data);
  let newCard = {...data}
  //console.log("Over 0.5: This is the card: ", data)
    ...
    <div>
      { !eventsWindowIsActive &&
        <div className={oddsWindowStyle} id={newCard.id} onClick={(e) => toggleEditCardWindow(newCard.id)}>
          ...
          <div className={style.grp}>
            <h3>Prediction</h3>
            <p>{data.over_zero_point_five.prediction}</p>
          </div>

          <div className={style.grp}>
            <h3>Percentage</h3>
            <p>{data.over_zero_point_five.percentage}%</p>
          </div>
          ...
          <div className={style.grp + " " + style.odds_grp}>
            ...
            <div className={style.odds}>
              <div className={style.odd}>
                <h4>Over 0.5</h4>
                <p>{data.over_zero_point_five.odds.over}</p>
              </div>
              <div className={style.odd}>
                <h4>Under 0.5</h4>
                <p>{data.over_zero_point_five.odds.under}</p>
              </div>              
            </div>
          </div>
          ...
          <div className={style.grp_scores}>
            ...
            {data.over_zero_point_five.isWon && <div className={style.validation_green}></div>}

            {!data.over_zero_point_five.isWon && <div className={style.validation_red}></div>}
          </div>
        </div>
      }
