...
function OverFourPointFive({data}) {
  ...
  //console.log('edit info over 4.5 data: ',data)
  ...
  const handlePredictionCode = (e) =>{
    ...
    if(code === "1"){
      newCard.over_four_point_five.prediction = 'over 4.5'
    }
    if(code === "0"){
      newCard.over_four_point_five.prediction = 'under 4.5'
    }    
  } 
  ...
  return (
        ...
        <form onClick={e=> removeCard(newCard.id)} className={styleList}>
          ...
          <div className={style.form_prediction}>
            ...
            <select
            onChange={e=>{
              newCard.over_four_point_five.prediction_code = e.target.value;
              const validatedCard = validateScores(newCard);
              updateEditedEvent(validatedCard)
            ...
            value={newCard.over_four_point_five.prediction_code}
            placeholder="prediction"
            >
              <option value="1">Over 4.5</option>
              <option value="0">Under 4.5</option>     
            </select>
          </div>
          ...
          <div className={style.form_percentage}>
            ...
            <input
            onChange={e=>{
              newCard.over_four_point_five.percentage = +e.target.value
              const validatedCard = validateScores(newCard);
              updateEditedEvent(validatedCard)
            }} 
            onBlur={()=> updateEditedEvent(newCard)}
            type="text" 
            value={newCard.over_four_point_five.percentage}
          ...
          <div className={style.form_odds}>
            ...
            <div className={style.odds}>
              <div className={style.odd}>
                ...
                <input 
                onChange={e=> {
                  newCard.over_four_point_five.odds.over = +e.target.value
                  const validatedCard = validateScores(newCard);
                  updateEditedEvent(validatedCard)
                }}
                onBlur={()=> updateEditedEvent(newCard)}
                type="number" 
                step="2"
                value={newCard.over_four_point_five.odds.over}
              ...
              <div className={style.odd}>
                ...
                <input 
                onChange={e=> {
                  newCard.over_four_point_five.odds.under = +e.target.value
                  const validatedCard = validateScores(newCard);
                  updateEditedEvent(validatedCard)
                }}
                onBlur={()=> updateEditedEvent(newCard)}
                type="number"
                step="2" 
                value={newCard.over_four_point_five.odds.under}
