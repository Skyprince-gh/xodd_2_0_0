...
function WinDrawWin({data}) {
  ...
  const handlePredictionCode = (e) =>{
    ...
    if(code === "1"){
      newCard.win_draw_win.prediction = 'home win'
    }
    else if(code === "2"){
      newCard.win_draw_win.prediction = 'away win'
    }
    else{
      newCard.win_draw_win.prediction = 'draw'
    }
  } 
  ...
  return (
        ...
        <form onClick={e=> removeCard(newCard.id)} className={styleList}>
          ...
          <div className={style.form_prediction}>
            ...
            <select
            onChange={e=>{
              newCard.win_draw_win.prediction_code = e.target.value;
              const validatedCard = validateScores(newCard);
              updateEditedEvent(validatedCard)
            ...
            value={newCard.win_draw_win.prediction_code}
          ...
          <div className={style.form_percentage}>
            ...
            <input
            onChange={e=>{
              newCard.win_draw_win.percentage = +e.target.value
              const validatedCard = validateScores(newCard);
              updateEditedEvent(validatedCard)
            }} 
            onBlur={()=> updateEditedEvent(newCard)}
            type="text" 
            value={newCard.win_draw_win.percentage}
          ...
          <div className={style.form_odds}>
            ...
            <div className={style.odds}>
              <div className={style.odd}>
                ...
                <input 
                onChange={e=> {
                  newCard.win_draw_win.odds.home = +e.target.value
                  const validatedCard = validateScores(newCard);
                  updateEditedEvent(validatedCard)
                }}
                onBlur={()=> updateEditedEvent(newCard)}
                type="number" 
                step="2"
                value={newCard.win_draw_win.odds.home}
                placeholder="home" />
              </div>
              <div className={style.odd}>
                <h4>Draw</h4>
                <input 
                onChange={e=> {
                  newCard.win_draw_win.odds.draw = +e.target.value
                  const validatedCard = validateScores(newCard);
                  updateEditedEvent(validatedCard)
                }}
                onBlur={()=> updateEditedEvent(newCard)}
                type="number"
                step="2" 
                value={newCard.win_draw_win.odds.draw}
                placeholder="draw" />
              </div>
              <div className={style.odd}>
                <h4>Away</h4>
                <input 
                onChange={e=> {
                  newCard.win_draw_win.odds.away = +e.target.value
                  const validatedCard = validateScores(newCard);
                  updateEditedEvent(validatedCard)
                }}
                onBlur={()=> updateEditedEvent(newCard)}
                type="number"
                step="2" 
                value={newCard.win_draw_win.odds.away}
                placeholder="away" />
              </div>
            </div>
          </div>
