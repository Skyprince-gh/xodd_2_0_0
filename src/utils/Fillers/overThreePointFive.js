...
const fill = async (convertedBatch, events) => {
  ...
  mergedBatches.forEach(entry => { // loop throgh the entire batch
    ...
    current.over_three_point_five.prediction = entry.prediction.toLowerCase();

    if (entry.prediction.toLowerCase().includes('over 3.5')) {
      current.over_three_point_five.prediction_code = '1';
    }
    else if (entry.prediction.toLowerCase().includes('under 3.5')) {
      current.over_three_point_five.prediction_code = '0';
    }
