...
const fill = async (convertedBatch, events) => {
  ...
  console.log('converted Batch over zero point five activated:', convertedBatch, events);
  ...
  const remainingBatches = convertedBatch.filter(batchObject => {
    if (batchObject.category === 'over-under-0.5') {
  ...
  mergedBatches.forEach(entry => { // loop throgh the entire batch
    ...
    current.over_zero_point_five.percentage = +entry.percentage.split('%')[0]; //get the percentage in string format and then convert it to number after spliting the % sign from it
    current.over_zero_point_five.odds.over = +entry.odds.over;
    current.over_zero_point_five.odds.under = +entry.odds.under;
    // current.over_zero_point_five.prediction = entry.prediction.toLowerCase();

    // if (entry.prediction.toLowerCase().includes('over 0.5')) {
    //   current.over_zero_point_five.prediction_code = '1';
    // }
    // else if (entry.prediction.toLowerCase().includes('under 0.5')) {
    //   current.over_zero_point_five.prediction_code = '0';
    // }
